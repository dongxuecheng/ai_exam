...
STREAM_CONFIGS = [
    ...
    StreamConfig(
        ...
        target_models={2,3}  # 顶部视角，用pose或者seg检查人，目标检测（警戒区，刷子，安全带）,seg来判断吊篮区域
    )
]
...
BASKET_K2_CONFIG= ServerConfig(
    ...
    server_port=5005,
    weights_paths=[
        os.path.join(WEIGHTS_BASE_DIR, 'yolo11l-pose1.pt'),
        os.path.join(WEIGHTS_BASE_DIR, 'yz_oil_tank.pt'),
        os.path.join(WEIGHTS_BASE_DIR, 'yolo11l-pose2.pt'),       
        os.path.join(WEIGHTS_BASE_DIR, 'yz_grounding_wire.pt')
    ],
    images_dir=IMAGES_DIR,
    static_mount_path='/basket_k2',#url中的路径
    img_url_path=f'http://127.0.0.1:5005/basket_k2',
